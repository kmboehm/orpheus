...
class SimpleAttention(pl.LightningModule):
    def __init__(
        ...
        setattr(self, "val_oncotype_score_concord", torchmetrics.ConcordanceCorrCoef())
        setattr(self, "trn_oncotype_score_concord", torchmetrics.ConcordanceCorrCoef())
        setattr(self, "val_oncotype_score_pearson", torchmetrics.PearsonCorrCoef())
        setattr(self, "trn_oncotype_score_pearson", torchmetrics.PearsonCorrCoef())
    ...
    def training_step(self, batch, batch_idx):
        ...
        self._update_metrics("trn", y_hat, batch["y"])
        self._log_metrics("trn")
        return loss
    ...
    def _update_metrics(self, subset, preds, targets_):
        ...
        pearson_metric = getattr(self, f"{subset}_oncotype_score_pearson")
        concord_metric = getattr(self, f"{subset}_oncotype_score_concord")
        pearson_metric(preds, targets)
        concord_metric(preds, targets)

    def _log_metrics(self, subset):
        pearson_metric = getattr(self, f"{subset}_oncotype_score_pearson")
        concord_metric = getattr(self, f"{subset}_oncotype_score_concord")
        self.log(
            f"{subset}_oncotype_score_pearson",
            pearson_metric,
            on_step=False,
            on_epoch=True,
            sync_dist=True,
        ...
        self.log(
            f"{subset}_oncotype_score_concord",
            concord_metric,
            on_step=False,
            on_epoch=True,
            sync_dist=True,
...
class TensorFusionNetwork(SimpleAttention):
    ...
    def forward(self, batch, return_emb=False):
        # if batch["report_emb"].ndim == 3:
        #     batch["report_emb"] = batch["report_emb"].squeeze(0)
        # if batch["slide_emb"].ndim == 3:
        #     batch["slide_emb"] = batch["slide_emb"].squeeze(0)
...
#         setattr(self, "val_score_concord", torchmetrics.ConcordanceCorrCoef())
#         setattr(self, "trn_score_concord", torchmetrics.ConcordanceCorrCoef())
#         setattr(self, "val_score_pearson", torchmetrics.PearsonCorrCoef())
#         setattr(self, "trn_score_pearson", torchmetrics.PearsonCorrCoef())
...
#     def calculate_loss(self, y_hat, target):
#         loss = nn.functional.mse_loss(y_hat, target, reduction="none").float()
#         loss = loss.mean()
#         return loss
...
#     def training_step(self, batch, batch_idx):
#         y_hat = self.forward(batch)
#         loss = self.calculate_loss(y_hat, batch["target"])
#         self.log("train_loss", loss.item(), on_step=True, on_epoch=True, sync_dist=True)
#         self._update_metrics("trn", y_hat, batch["target"])
#         self._log_metrics("trn")
#         return loss
...
#         loss = self.calculate_loss(y_hat, batch["target"])
#         self.log("val_loss", loss.item(), on_step=True, on_epoch=True, sync_dist=True)
#         self._update_metrics("val", y_hat, batch["target"])
#         self._log_metrics("val")
#         return loss.item()

#     def _update_metrics(self, subset, preds, targets_):
#         targets = targets_.view(-1, 1)
#         pearson_metric = getattr(self, f"{subset}_score_pearson")
#         concord_metric = getattr(self, f"{subset}_score_concord")
#         pearson_metric(preds, targets)
#         concord_metric(preds, targets)
